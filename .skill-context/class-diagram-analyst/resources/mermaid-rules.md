...
> Nguồn: Mermaid v10+ Official Documentation
> Mục đích: Syntax whitelist cho class-diagram-analyst
> Builder đọc file này để tạo `knowledge/mermaid-rules.md`
...
```
+string email          ← lowercase type (Mermaid case-sensitive nhưng OK, tuy nhiên dùng PascalCase cho nhất quán)
+email: String         ← sai thứ tự, không dùng colon
+String email = ""     ← không hỗ trợ default value trong field declaration
```
...
| `<<Collection>>` | PayloadCMS collection (custom) |
| `<<EmbeddedDoc>>` | Embedded document (custom) |
...
| Relationship arrow sai chiều | `Post --|> User` (sai) | `Post --> User : authoredBy` |
...
## 11. Mermaid Version Compatibility

Skill này target **Mermaid v10+** (phiên bản phổ biến trong 2024-2026).

Features CÓ trong v10+:
- `namespace` grouping
- `note for ClassName`
- Full cardinality syntax

Features KHÔNG CÓ hoặc unstable:
