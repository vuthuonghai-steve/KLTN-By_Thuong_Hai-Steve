...
sequenceDiagram
    ...
    Note over A: Phase 0 — Context Boot (autonomous)
    A->>K: Read SKILL.md, pencil-tools-ref.md, project-context.md
    A->>FS: Read CLAUDE.md, check.list.md, Docs/life-2/ui/specs/ index
    A->>Pen: get_editor_state() → verify STi.pen open
    A->>A: Build project_context (module scope, design rules, canvas state)

    Note over A: Phase 1 — Spec Analyzer (autonomous)
    A->>FS: Read spec file
    A->>A: Parse → extract Screens[], States, Component Map
    A->>L: Self-verify: all screens extracted? citations present?
    Note over A: [Auto-proceed — no human gate]

    Note over A: Phase 2 — Wireframe Blueprint (autonomous)
    ...
    Note over A: Phase 3 — Pencil Drawer (autonomous self-correcting loop)
    loop For each screen in screens[]
        A->>Pen: batch_get Lib-Component → collect real node IDs
        A->>Pen: find_empty_space_on_canvas → get safe position
        A->>Pen: batch_design I/C/U → draw screen
        A->>Pen: get_screenshot → visual verify
        ...
        else Fail — fixable (overlap / wrong node)
            A->>Pen: Fix → redraw (max 2 retries)
        else Fail — blocked (component missing, no fallback)
            A->>U: ⚠️ ESCALATE — report blocker + proposed resolution
            U->>A: Decision
        end
    end
    A->>U: ✅ Done — summary report (N screens drawn, M warnings)
```
...
flowchart LR
    ...
    subgraph P1 [Phase 1: Spec Analyzer — Autonomous]
        S1A[Read spec file] --> S1B[Extract Screens + States + Component Map]
        S1B --> S1C{Self-verify:\nall extracted?}
        S1C -->|Pass| S1D[Auto-proceed ✅]
        S1C -->|Fail| S1E[⚠️ Escalate: spec unreadable]
    end

    subgraph P2 [Phase 2: Wireframe Blueprint — Autonomous]
        S2A[Load template + layout-rules] --> S2B[Generate DOM Tree per screen]
        S2B --> S2C[Save to wireframes/]
        S2C --> S2D{Self-verify:\nno hallucinations?}
        S2D -->|Pass| S2E[Auto-proceed ✅]
        S2D -->|Fail| S2F[Self-correct → re-generate]
    end

    subgraph P3 [Phase 3: Pencil Drawer — Autonomous Loop]
        S3A[batch_get Lib-Component] --> S3B[find_empty_space_on_canvas]
        S3B --> S3C[batch_design — draw 1 screen]
        S3C --> S3D[get_screenshot verify]
        S3D --> S3E{Self-verify:\nchecklist pass?}
        S3E -->|Pass| S3F{More screens?}
        S3F -->|Yes| S3B
        S3F -->|No| DONE
        S3E -->|Fail fixable| S3G[Self-fix → retry ≤2x]
        S3G --> S3C
        S3E -->|Fail blocked| S3H[⚠️ Escalate to User]
        S3H --> S3B
    end

    P0 --> P1 --> P2 --> P3
    DONE([✅ Summary report to User])
```
