...
\usepackage[a4paper,top=3.0cm,bottom=3.0cm,left=3.5cm,right=2.0cm]{geometry}
\usepackage{fontspec}
...
% Thay đổi ký hiệu danh sách (itemize) sang dấu gạch ngang theo văn phong tự nhiên
...
\begin{titlepage}
  ...
  {\fontsize{22pt}{28pt}\selectfont \textbf{NGHIÊN CỨU VÀ ỨNG DỤNG TRÍ TUỆ NHÂN TẠO (AI) TRONG PHÁT TRIỂN WEBSITE MẠNG XÃ HỘI}}\\[3.5em]
...
\begin{flushright}
  \begin{minipage}{8cm}
    \centering
    \textit{Hà Nội, ngày \dots\dots tháng \dots\dots năm 2026}\\[1.5em]
...
Trong bối cảnh trí tuệ nhân tạo đang tái định nghĩa quy trình phát triển phần mềm, khóa luận tập trung nghiên cứu và xây dựng hệ thống \textbf{Agent Skill Framework}—một bộ công cụ AI chuyên biệt hỗ trợ tự động hóa các giai đoạn phân tích và thiết kế hệ thống. Đề tài đề xuất kiến trúc \textit{3 Pillars} bao gồm Knowledge, Process, Guardrails, kết hợp với mô hình \textit{Meta-Skill} theo chu trình vận hành \texttt{Architect $\rightarrow$ Planner $\rightarrow$ Builder}. Cách tiếp cận này giúp tối ưu hóa việc tổ chức tri thức và quy trình làm việc cho AI Agent, từ đó giảm thiểu hiện tượng ảo giác (hallucination) và nâng cao tính nhất quán trong thiết kế phần mềm.

Hệ thống được kiểm chứng thông qua việc phát triển nền tảng \textbf{Steve Void}—một mạng xã hội chia sẻ tri thức dành cho cộng đồng lập trình viên. Ứng dụng được xây dựng trên các công nghệ hiện đại như \textit{Next.js 15}, \textit{Payload CMS 3.x} và \textit{MongoDB Atlas}. Khóa luận đã tích hợp chuỗi 28 Agent Skills chuyên biệt để tự động hóa việc vẽ các sơ đồ UML như Sequence, Activity, Flow, Class, đồng thời thiết kế lược đồ dữ liệu Schema và trích xuất các đặc tả giao diện (UI/UX).

Kết quả thực nghiệm cho thấy \textit{Agent Skill Framework} giúp tăng năng suất phát triển phần mềm trung bình từ 40\% đến 60\% trong giai đoạn thiết kế, đồng thời duy trì chất lượng tài liệu kỹ thuật theo các tiêu chuẩn công nghiệp. Nghiên cứu giải quyết bài toán tối ưu hóa quy trình làm việc cho doanh nghiệp, đồng thời mở ra một mô hình đào tạo thực tiễn cho sinh viên và người mới học lập trình trong kỷ nguyên trí tuệ nhân tạo.

\textbf{Từ khóa:} AI Agent, Agent Skill Framework, Next.js 15, Payload CMS, Steve Void, Software Engineering.
...
\begin{center}
  \begin{longtable}{|>{\centering\arraybackslash}p{0.15\textwidth}|p{0.40\textwidth}|p{0.35\textwidth}|}
    \hline
    \rowcolor[gray]{0.9} \textbf{Ký hiệu / Viết tắt} & \textbf{Tiếng Việt} & \textbf{Tiếng Anh} \\
    \hline
    \endfirsthead
    \hline
    \rowcolor[gray]{0.9} \textbf{Ký hiệu / Viết tắt} & \textbf{Tiếng Việt} & \textbf{Tiếng Anh} \\
    ...
    \hline
    \endfoot
    UC & Mô tả các chức năng hệ thống từ góc nhìn người dùng & Use Case \\ \hline
    US & Mô tả yêu cầu của người dùng dưới dạng ngắn gọn & User Story \\ \hline
    SRS & Tài liệu mô tả đầy đủ các yêu cầu của phần mềm & Software Requirement Specification \\ \hline
    AI & Công nghệ mô phỏng trí tuệ con người trong máy tính & Artificial Intelligence \\ \hline
    LLM & Mô hình AI xử lý và sinh văn bản dựa trên dữ liệu lớn & Large Language Model \\ \hline
    API & Cơ chế cho phép hai ứng dụng giao tiếp với nhau & Application Programming Interface \\ \hline
    REST & Phong cách kiến trúc thiết kế API dựa trên HTTP & Representational State Transfer \\ \hline
    CSDL & Hệ thống lưu trữ và quản lý dữ liệu có tổ chức & Database \\ \hline
    CMS & Phần mềm giúp tạo, quản lý và xuất bản nội dung website & Content Management System \\ \hline
    UML & Ngôn ngữ chuẩn để trực quan hóa thiết kế phần mềm & Unified Modeling Language \\ \hline
    ERD & Sơ đồ biểu diễn mối quan hệ giữa các bảng dữ liệu & Entity Relationship Diagram \\ \hline
    UI/UX & Thiết kế giao diện và trải nghiệm người dùng & User Interface / User Experience \\ \hline
    CRUD & Bốn thao tác cơ bản với dữ liệu: Tạo, Đọc, Sửa, Xóa & Create, Read, Update, Delete \\ \hline
    DTO & Cấu trúc dữ liệu dùng để truyền thông tin giữa các tầng & Data Transfer Object \\ \hline
    YAML & Định dạng dữ liệu dễ đọc, thường dùng cho file cấu hình & YAML Ain't Markup Language \\ \hline
    JSON & Định dạng dữ liệu văn bản phổ biến trong lập trình web & JavaScript Object Notation \\ \hline
    CLI & Giao diện điều khiển hệ thống bằng dòng lệnh văn bản & Command Line Interface \\ \hline
    SSR & Kỹ thuật render HTML trên server trước khi gửi về client & Server-side Rendering \\ \hline
    CSR & Kỹ thuật render giao diện hoàn toàn trên trình duyệt & Client-side Rendering \\ \hline
    SEO & Tối ưu website để xếp hạng cao trên công cụ tìm kiếm & Search Engine Optimization \\ \hline
    RSC & Thành phần React được render trực tiếp trên server & React Server Components \\ \hline
    SSE & Công nghệ truyền dữ liệu từ server đến client qua HTTP & Server-Sent Events \\ \hline
    ASF & Khung làm việc định nghĩa và quản lý các kỹ năng AI & Agent Skill Framework \\ \hline
    MSF & Khung quản lý cấp cao cho việc tổ chức và điều phối các skill & Meta-Skill Framework \\ \hline
    IP & Vị trí tương tác giữa người dùng và hệ thống trong workflow & Interaction Point \\ \hline
    Schema & Cấu trúc logic định nghĩa cách tổ chức dữ liệu trong CSDL & Database Schema \\ \hline
    Wireframe & Bản phác thảo giao diện thể hiện bố cục và chức năng & Wireframe Blueprint \\ \hline
    Guardrail & Cơ chế kiểm tra và đảm bảo chất lượng đầu ra của AI & Safety Guardrail \\ \hline
  \end{longtable}
\end{center}
...
Trong hơn 10 năm hoạt động, Innotech đã triển khai thành công hơn 100 dự án cho các doanh nghiệp và tập đoàn lớn như Kiến Tạo Xanh Group, ITM Group, OVAN Group, HANOTEX,... Với đội ngũ hơn 50 chuyên gia giàu kinh nghiệm, công ty luôn giữ vững các giá trị cốt lõi về sự chuyên nghiệp, tính linh hoạt, bảo mật và hiệu quả kinh tế cho đối tác.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Phân tích các mô hình tác vụ AI hiện đại như: Mô hình Phản hồi Hành động, Sử dụng công cụ, và Hệ thống đa tác vụ.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Survey \& Interview:} Khảo sát định lượng với thang Likert và phỏng vấn định tính với người dùng
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Giúp sinh viên hiểu rõ "tại sao" chứ không chỉ "làm thế nào" thông qua explanations trong Skills
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Chuẩn hóa quy trình làm việc: mọi developer trong team sử dụng chung một bộ Skills, giúp đồng nhất phương pháp trước khi tiến hành sản xuất.
...
\begin{itemize}
    ...
    \item \textbf{Khả năng sinh nội dung (\textit{Generation}):} Từ các câu lệnh dẫn dắt (\textit{prompt}) đầu vào, LLMs có thể sinh ra mã nguồn, tài liệu thiết kế, sơ đồ UML và các thành phẩm kỹ thuật (\textit{artifacts}) khác.
...
\begin{itemize}
    ...
    \item \textbf{Tối ưu hóa tìm kiếm (\textit{SEO Optimization}):} Cơ chế xử lý dữ liệu đặc tả (\textit{Metadata API}) kết hợp kết xuất phía máy chủ giúp nội dung được các bộ máy tìm kiếm chỉ mục hóa hiệu quả.
\end{itemize}
...
Quy tắc thiết kế theo phong cách \textbf{Neobrutalism}: đường viền đậm, bóng đổ lệch, tông màu tương phản cao với màu chủ đạo là Pink.
...
\begin{itemize}
    ...
    \item \textbf{Xem trước thời gian thực:} Nhờ cấu trúc tệp \texttt{.md}, các sơ đồ có thể hiển thị trực quan ngay trên các trình soạn thảo (như VS Code) hoặc các nền tảng quản lý mã nguồn, duy trì tính nhất quán trong thiết kế.
...
Việc kết hợp AI vào quy trình phân tích và thiết kế giúp chuyển đổi các yêu cầu nghiệp vụ phức tạp thành các Agent Skills chuyên biệt, tạo nền tảng kiến trúc vững chắc cho hệ thống.
...
Hệ thống Agent Skills được xây dựng dựa trên mô hình \textbf{Meta-Skill Framework}, bao gồm ba trụ cột chính (3 Pillars) và bảy vùng chức năng (7 Zones). Kiến trúc này hướng tới tính nhất quán, khả năng kiểm soát chất lượng và khả năng mở rộng của các bộ Agent Skill.
...
Ba giai đoạn này được kết nối với vòng lặp kiểm soát (Verify/Fix/Loop) nhằm kiểm chứng chất lượng đầu ra. Mô hình được minh họa trong Hình~\ref{fig:meta-skill-framework}.
...
Phiên bản Skill Architect v2 giới thiệu cơ chế Adaptive Workflow — tự động phân loại độ phức tạp của yêu cầu và chọn workflow tối ưu:

\begin{itemize}
    \item \textbf{Simple}: COLLECT (rút gọn) → DESIGN (gộp Analyze và Design).
    \item \textbf{Medium}: COLLECT → ANALYZE → DESIGN.
    \item \textbf{Complex}: COLLECT → ANALYZE → ARCH-REVIEW → DESIGN.
\end{itemize}

Việc phân loại độ phức tạp dựa trên: số Zones cần dùng, số file cần tạo, mức độ phụ thuộc với các skill khác và độ mới của miền nghiệp vụ. Hình~\ref{fig:skill-arch-adaptive} thể hiện 3 nhánh này với Gate IP-4 bắt buộc trước khi bàn giao.
...
Nhằm kiểm chứng chất lượng thiết kế, Skill Architect tích hợp cơ chế Self-Scoring — AI tự đánh giá từng section của \texttt{design.md} theo thang điểm 1-5. Các tiêu chí đánh giá bao gồm: độ rõ ràng, tính đầy đủ, tính khả thi, và tuân thủ chuẩn.
...
\begin{enumerate}
    ...
    \item \textbf{Progressive Disclosure Tiering}: Phân biệt rõ Tier 1 bắt buộc và Tier 2 tùy chọn
...
Flow Design Analyst chuyên trách vẽ Business Process Flow Diagram với Swimlane 3-lane (User / System / DB), yêu cầu mọi bước logic phải có căn cứ từ spec hoặc User Story.
...
Sequence Design Analyst chuyên vẽ Sequence Diagram chuẩn UML, mô tả tương tác giữa các đối tượng theo thời gian.

\subsection{Cấu trúc thư mục}

Hình~\ref{fig:seq-folder} thể hiện cấu trúc thư mục tối giản của Sequence Design Analyst — chỉ cần 4 zones thiết yếu: Core, Knowledge, Templates và Loop.
...
Hình~\ref{fig:seq-exec} minh họa luồng thực thi đầy đủ giữa Steve (user), Analyst Skill, Codebase, Knowledge Zone và Loop Zone, bao gồm nhánh xử lý khi thông tin đầy đủ và khi thiếu thông tin.
...
Activity Diagram Analyst vẽ sơ đồ hoạt động theo tư duy Clean Architecture (Business-UseCase-External), phát hiện Deadlocks và kiểm tra tính nhất quán giữa nghiệp vụ và thiết kế.
...
\begin{itemize}
    ...
    \item \textbf{Làn Hạ tầng (\textit{Infrastructure Lane}):} Chứa các bộ lưu trữ dữ liệu và dịch vụ bên ngoài.
\end{itemize}
...
\subsection{Ràng buộc chống ảo giác}
...
\subsection{5 Guardrails kiểm soát độ chính xác}
...
Người dùng cung cấp đầu vào (tên module, use case ID, hoặc mô tả mơ hồ), Agent Skill tự động khám phá tài nguyên dự án (specs, user stories, ER diagram), trích xuất logic nghiệp vụ và sinh đầu ra chuẩn hóa. Các điểm tương tác (Interaction Points) cho phép người dùng kiểm soát chất lượng tại từng bước quan trọng.
...
Mỗi Activity Diagram sử dụng Mermaid Flowchart TD (Top-Down) với 3 swimlanes: \textbf{User Lane}, \textbf{System Lane} và \textbf{DB Lane}. Các sơ đồ bao gồm đầy đủ: điều kiện rẽ nhánh, xử lý lỗi và điểm kết thúc rõ ràng cho mọi nhánh.

\subsection{Kết quả sơ đồ tuần tự (Sequence Diagrams)}

Sequence Design Analyst tạo ra \textbf{9 sơ đồ tuần tự}, mỗi sơ đồ mô tả tương tác chi tiết giữa các thành phần hệ thống theo thời gian. Các thành phần tham gia bao gồm: \texttt{User}, \texttt{UI (Next.js/React)}, \texttt{Service}, \texttt{Payload CMS}, \texttt{DB (MongoDB)}, \texttt{SSE Dispatcher} và \texttt{Mailer}.
...
Các sơ đồ sử dụng đầy đủ UML fragments chuẩn: \texttt{alt}, \texttt{opt}, \texttt{loop} và \texttt{activate/deactivate} blocks để thể hiện thời gian xử lý. Nguyên tắc \textbf{Code-First Truth} đảm bảo tên các đối tượng trong sơ đồ khớp 100\% với codebase dự kiến.
...
Đặc biệt, mọi Flow Diagram đều được gắn mã \textbf{UC-ID} (Use Case ID) để phục vụ việc truy xuất nguồn gốc ngược lại yêu cầu chức năng. Guardrail \textbf{G1 --- No Blind Step} của Flow Design Analyst quy định: không có bước nào được thêm vào sơ đồ mà không có căn cứ từ spec hoặc user story.
...
Mỗi trường trong Class Diagram đều có \textbf{Source Citation} — truy xuất nguồn gốc tới ER Diagram hoặc Activity Diagram cụ thể. Guardrail \textbf{Source Citation Mechanism} bảo đảm: field không có source → BLOCK, không ghi file.
...
\hline
Chỉ cho phép ghi thêm (\textit{Append-Only}) & Nhật ký hệ thống không cho phép thao tác sửa hoặc xóa, bảo toàn tính vẹn toàn của lịch sử truy vết (\textit{audit trail}). \\
\hline
Chỉ mục hỗn hợp (\textit{Compound Index}) & Áp dụng các ràng buộc duy nhất trên nhiều trường dữ liệu kết hợp để bảo đảm tính toàn vẹn (ví dụ: cặp mã bài viết và mã người dùng cho lượt thích). \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{0 trường ảo giác} — mọi field đều có nguồn từ Contract YAML
\end{itemize}
...
Đặc biệt, mọi component trong wireframe đều có thuộc tính \texttt{spec-cite} trỏ tới section cụ thể trong UI Spec (ví dụ: \texttt{[spec §2.1 — input-email]}). Guardrail \textbf{G-Spec-Strict} của UI Pencil Drawer cấm thêm bất kỳ element nào mà spec không đề cập.
...
Chuỗi truy xuất nguồn gốc (\textit{Traceability Chain}) giữa các thành phẩm kỹ thuật duy trì tính liên tục xuyên suốt toàn bộ vòng đời:
...
\begin{itemize}
    ...
    \item \textbf{0 trường ảo giác} trong toàn bộ Schema Design — mọi field đều có nguồn từ Contract YAML
...
\begin{itemize}
    ...
    \item Hooks specification (beforeChange, afterChange) duy trì tính nhất quán dữ liệu khi triển khai.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Wireframe chưa bao gồm responsive đầy đủ}: Chỉ có 2/17 wireframes có biến thể cho máy tính, các wireframe còn lại chủ yếu thiết kế cho thiết bị di động
...
\addcontentsline{toc}{section}{Hạn chế của nghiên cứu}
...
\addcontentsline{toc}{section}{Hướng phát triển tiếp theo}
...
\begin{enumerate}
    ...
    \item \textbf{Tích hợp đa mô hình AI (Multi-Model Architecture):} Thay vì phụ thuộc vào một mô hình AI duy nhất, nghiên cứu hướng tới việc tích hợp nhiều mô hình AI khác nhau (GPT-4, Gemini, open-source models) cho từng loại tác vụ cụ thể, nhằm tối ưu chi phí và tận dụng thế mạnh riêng của mỗi mô hình.

    \item \textbf{Xây dựng hệ thống đánh giá tự động (Automated Quality Assessment):} Phát triển bộ test tự động để đánh giá chất lượng đầu ra của Agent Skills: kiểm tra tính đúng đắn của sơ đồ, tính nhất quán giữa các artifacts, và so sánh với benchmark từ các dự án thực tế.
...
Kết quả nghiên cứu cho thấy tiềm năng rõ rệt của việc ứng dụng AI Chatbot vào phát triển phần mềm, đặc biệt ở giai đoạn phân tích và thiết kế. Mặc dù còn những hạn chế về kinh phí, thời gian và độ sâu nghiệp vụ, kết quả đạt được đặt nền móng cho một hướng đi đầy triển vọng, nơi AI không còn là công cụ hỗ trợ đơn thuần mà dần trở thành \textbf{cộng tác viên kỹ thuật} thực sự trong quy trình phát triển phần mềm.
...
Cấu trúc này giúp mỗi skill vận hành như một đơn vị độc lập, tự chứa đầy đủ kiến thức miền, quy trình làm việc và cơ chế kiểm soát chất lượng.

\section*{Phụ lục B: Trích mẫu SKILL.md}
...
- **Đặc điểm:** Kiến trúc sư UML cấp cao. Thiết kế sơ đồ 
...
- Xác định Kịch bản chính, các tác nhân, các đường đời
...
- Thiết kế các phân đoạn: rẽ nhánh, lặp, tùy chọn
